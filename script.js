...
let i=0;
const role=document.getElementById('roleWord');
if(role){
  setInterval(()=>{
    i=(i+1)%words.length;
    role.animate([{opacity:1},{opacity:0}],{duration:160,fill:'forwards'}).onfinish=()=>{
      role.textContent=words[i];
      role.animate([{opacity:0},{opacity:1}],{duration:230,fill:'forwards'});
    };
  },2400);
}
